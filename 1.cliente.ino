...
const int dataIn          = 16;   //Pino 14 Data  74HC595                                                       - D0 no ESP8266   D16 no ESP32
...
const int led_wifi        = 1;    //LED que indica a conexão com o wifi                                         - TX no ESP9266   TX0 no ESP32
const int led_db          = 17;   //LED que indica se o sistema está energizado                                 - -- no ESP8266   TX2 no ESP32

//const int ploadPin        = 5;    //Conecta ao pino 1 do 74HC165 (PL - asynchronous parallel load input)(SH/LD) - D1  no ESP8266
//const int clockEnablePin  = 2;    //Conecta ao pino 15 do 74HC165 (CE - Clock Enable Input)(CLK INH)            - D4  no ESP8266
//const int dataPin         = 4;    //Conecta ao pino 9 do 74HC165 (Q7 - serial output from the last stage)(QH)   - D2  no ESP8266
//const int clockPin        = 15;   //Conecta ao pino 2 do 74HC165 (CP - Clock Input)(CLK)                        - D8  no ESP8266
//const int clockIn         = 21;   //Pino 11 Clock 74HC595                                                       - --  no ESP8266
//const int latchPin        = 17;   //Pino 12 Latch 74HC595                                                       - --  no ESP8266
//const int dataIn          = 16;   //Pino 14 Data  74HC595                                                       - D0  no ESP8266
//const int led_mqtt        = 19;   //LED que indica o status da conexão com o broker MQTT                        - --  no ESP8266
//const int led_wifi        = 23;   //LED que indica a conexão com o wifi                                         - --  no ESP8266
//const int led_power       = 22;   //LED que indica se o sistema está energizado                                 - --  no ESP8266
//const int led_db          = 8;    //LED que indica o status da conexão com o banco de dados                     - SD1 no ESP8266
...
WiFiClient client;
//MySQL_Connection conn((Client *)&client);                                         DB
...
//void conectaBD();         //Conecta no Banco de Dados                             DB
//void enviaBD();                                                                   DB
...
//    pinMode(led_db, OUTPUT);                                                      DB
    pinMode(led_wifi, OUTPUT);
    pinMode(led_power, OUTPUT);
    
    digitalWrite(led_mqtt, LOW);
//    digitalWrite(led_db, LOW);                                                    DB
...
void read_shift_regs()
{
    ...
    for(int i = 0; i < nExtIn; i++)
    {
        ...
        if(!button_cmd_old[i] && button_cmd[i]){
          ...
          enviaValoresMQTT();
//          enviaBD(x, (int)button_cmd[i], 0);                                DB
...
//    enviaBD(x, (int)button_cmd[i], 1);                                      DB
